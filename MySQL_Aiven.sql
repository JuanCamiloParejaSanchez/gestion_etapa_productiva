...
CREATE TABLE IF NOT EXISTS `aprendices` (
  ...
  `bloqueadoHasta` datetime DEFAULT NULL COMMENT 'Fecha hasta la cual la cuenta está bloqueada',
  ...
  KEY `idx_estado_formacion` (`estadoFormacion`),
...
DROP TABLE IF EXISTS documentos_aprendiz;

CREATE TABLE `documentos_aprendiz` (
  `id` int NOT NULL AUTO_INCREMENT,
  `aprendiz_id` int NOT NULL,
  `tipo_documento` enum('carta_presentacion','arl','acuerdo_inicio','informe_final','evaluacion','certificado','otro') NOT NULL,
  ...
  CONSTRAINT `fk_documentos_aprendiz` FOREIGN KEY (`aprendiz_id`) REFERENCES `aprendices` (`id`) 
    ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `fk_documentos_revisor` FOREIGN KEY (`revisado_por`) REFERENCES `administradores` (`id`) 
    ON DELETE SET NULL ON UPDATE CASCADE,
  CONSTRAINT `chk_tamano_archivo` CHECK (`tamano_bytes` > 0 AND `tamano_bytes` <= 10485760)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci 
COMMENT='Documentos subidos por los aprendices';
...
-- Ver estructura de documentos_aprendiz
SHOW CREATE TABLE documentos_aprendiz;

-- Ver definición de la vista
SHOW CREATE VIEW v_documentos_pendientes_revision;

-- 1. Ver la conexión actual
SELECT @@hostname, @@port;

-- 2. Ver todas las tablas creadas
SHOW TABLES;
...
-- 4. Contar registros de las tablas principales
SELECT 
    (SELECT COUNT(*) FROM aprendices) as total_aprendices,
    (SELECT COUNT(*) FROM administradores) as total_administradores,
    (SELECT COUNT(*) FROM bitacoras) as total_bitacoras,
    (SELECT COUNT(*) FROM documentos_aprendiz) as total_documentos;

-- Ejecuta esto en MySQL Workbench:
SELECT 
    @@hostname as 'Servidor_Aiven',
    @@port as 'Puerto',
    DATABASE() as 'Base_Datos',
    USER() as 'Usuario',
    (SELECT COUNT(*) FROM information_schema.TABLES WHERE TABLE_SCHEMA = 'defaultdb') as 'Total_Tablas';